...
<template>
  <div class="text">
    <div class="emoji">
      <i class="icon iconfont icon-look" @click="showEmoji=!showEmoji" />
      <transition name="showbox">
        <div v-show="showEmoji" class="emojiBox">
          <li v-for="item in emojis" :key="item.id">
            <img :src="require(&quot;../../../public/emoji/&quot;+item.file)" :data="item.code" @click="content +=item.code">
          </li>
        </div>
      </transition>
    </div>
    ...
    send() {
      if (this.content.length <= 1) {
        ...
        if (this.selectedChat.user.name === '机器人') {
          ...
          this.$store.dispatch('sendMessage', msg)
          this.$socket.send({ sender_user_id: this.user.user_id, receiver_user_id: this.selectedChat.user.user_id, message: this.content })
          console.log({ user_id: this.selectedChat.user.user_id, message: this.content })
          this.content = ''
        }
      }
    }
