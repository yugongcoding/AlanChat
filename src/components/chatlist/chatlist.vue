...
<template>
  <div class="msglist">
    <ul>
      <li v-for="item in searchedChatlist" :key="item.id" class="sessionlist" :class="{ active: item.id === selectId }" @click="selectSession(item.id)">
        <div class="list-left">
      ...
      if (!data) {
        this.$axios
          ...
          .then((response) => {
            ...
            this.$store.dispatch('initChatList', data)
            console.log(data)
          })
      }
    }
